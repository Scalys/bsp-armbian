	...
	for release in $target_release; do
		for arch in $target_arch; do
			...
			for plugin in "${SRC}"/packages/extras-buildpkgs/*.conf; do
				...
				if [[ $(type -t package_fetch) == function ]]; then
					mkdir -p "${SRC}/cache/sources/extra/$package_name"
					package_fetch "${SRC}/cache/sources/extra/$package_name"
				else
					fetch_from_repo "$package_repo" "extra/$package_name" "$package_ref"
				fi

				mount_chroot "${target_dir}"
