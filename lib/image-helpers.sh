...
install_deb_chroot()
{
	...
	[[ $BUILD_ALL == yes && ${variant} == remote ]] && chroot "${root}" /bin/bash -c "DEBIAN_FRONTEND=noninteractive apt-get $apt_extra -yqq update"
	chroot "${root}" /bin/bash -c "DEBIAN_FRONTEND=noninteractive apt-get -yqq $apt_extra --no-install-recommends install $name" >> "${DEST}"/${LOG_SUBPATH}/install.log 2>&1
